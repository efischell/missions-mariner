#!/bin/bash
# t. schneider tes@mit.edu 2012-06-16
...
if [[ $RUNTYPE == simprocesses ]]; then META_PREFIX=sim; MOOS_PREFIX=sim; fi
...
if [[ -z $MISSIONS_LAMSS_FLAGS ]]; then MISSIONS_LAMSS_FLAGS=""; fi
...
cat <<EOF
#################################################
missions-lamss launch utility v2
t. schneider tes@mit.edu 2012-06-16
...
for param in $@ $MISSIONS_LAMSS_FLAGS; do
    if [[ $param != -D* ]]; then
        check_param $param
        disp_param $param
        FLAGS="${FLAGS} -D$param"
